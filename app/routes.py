# -*- coding: utf-8 -*-
#------------------------------------------------------------------
# File with all the routes of the flask app
#
#
...
def text_delete():
    """
    Route for deleting a text from the database on submit of the form

    :return: template of text delete page
    :return: form for text deletion
    """
...
def text_download():
    """
    Route for download a text in Conll format

    :return: template for the download page
...
def term_upload():
    """
    Route for uploading terminology

    :return: template of terminology
    :return: form for uploading terms
    :return: all books in the database and their status of terminology
    """
...
def baseline():
    """
    Route for the aumatic extraction of prerequisites

    :return: template of baseline methods
    :return: form for selecting the method to launch
...
def get_status_baseline(book):
    """
        Get status of a book and create json for the visualization

        :param book: string with book id and chapter id
        :return json with baseline status
...
def get_status_burst(book):
    """
        Get status of the burst and create json for the visualization

        :param book: string with book id and chapter id
        :return json with burst status
    """
...
def get_status_params(book):
    """
        Get burst params of the selected book
...
def baseline_results():
    """
    Route for the visualization of the baseline results

    :return: template for viewing the results
    :return: form for selecting the method to visualize
...
def download_method_results(dati):
    """
    Function for downloading csv file of results of a selected book and method

    :param dati: string with book id, chapter id and method id
    :return: download of the baseline results
    """
    dati = dati.split(",")
...
def analysis():
    """
    Route for the analysis of the extracted graphs with:
    - 1: data summary
...
def get_annotation(book):
    """
    Get all the annotations of a book

    :param book: string with the book id and chapter id
    :return: json with all the annotations of the book
    """
    print(book)
    bid = book.split(",")[0]
    cap = book.split(",")[1]
...
def download_burst_summary(dati):
    """
    Function for downloading burst summary in csv format

    :param dati: string with data to download
    :return: download of csv format file
    """
    dati = dati.split(",")
...
@login_required
def comparison():
    """Route for the comparison between two annotations"""
    form = ComparisonForm()
    cap_lista = Conll.query.all()
    form.book_cap.choices = [(str(str(cap.bid) + ',' + str(cap.cap)), ("Title: " + (db.session.query(Book.title).join(Conll, cap.bid == Book.bid).first()).title + " -- Chapter: " + str(cap.cap))) for cap in cap_lista]
    if cap_lista:
...
def get_annotation_comparison(book):
    ...
    :param book: string with book id and chapter id
    :return: json with all the annotations
    """
    bid = book.split(",")[0]
    cap = book.split(",")[1]
...
def pre_annotator():
    """
    Route for selecting book and chapter to annotate

    :return: page for the annotation
    :return: file with all the text info
    """
...
def download_ann(dati):
    """
    Download csv file containing the annotation

    :param dati: strings with book id, chapter id and user id
    :return: csv with the selected annotation
    """
    dati = dati.split(",")
...
def annotation_upload():
    """
    Upload the partial annotation in the database
...
def final_annotation_upload():
    """
    Upload the final annotation in the database
...
def pre_revision():
    """
    Route for selecting the book to revise.

    :return: template of the book selection for the revision
    :return: form

    on form submit:
    :return: template of the revision page
    :return: file with all the relations of the selected annotation
...
def pre_visualization():
    """
    Route for selecting the annotation and method of visualization
    Visualization types:
    1 - matrix
    2 - arc diagram
    3 - graph
    4 - gantt (only for burst method)

    :return: template of the selected method
    """
...
def get_json_data(data):
    """
    Get json for the visualization of the selected graph

    :param data: string with book id, chapter id and user id
    :return: json
    """
    print(data)
    value = data.split(",")
...
def download_gantt(dati):
    """
    Download burst results in csv format

    :param dati: string with book id and chapter id
    :return: csv file
    """
    dati = dati.split(",")
...
def gold_standard():
    """
    Route for the creation of the gold standard

    :return: template for the creation of the GS
    :return: form
    :return: golds already in the database
    """
...
def download_gold(dati):
    """
    Function for download the gold standard in csv format
...
def get_annotations(book):
    """
    Get all the annotations for the creation of the gold standard
...
def get_agreements(agreements):
    """
    Get all the percentages of agreements possible

    :param agreements: list of all the selected annotations
    :return: json with the possible agreements
    """
    agreementsList = []
    ann = int(agreements)
    for i in range(ann):
